...
while running:
    ...
    for orbs in clusters:
        ...
        for orb in orbs:
            ...
            r_vec = orb.pos - mouse_pos
            distance = r_vec.length()
            if 1 < distance < effect_radius:
                tangential = pygame.math.Vector2(r_vec.y, -r_vec.x)
                tangential.normalize_ip()
                spinning_force = tangential * (F_ROT / (distance * distance))
                total_force += spinning_force
